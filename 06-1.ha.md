...
listen kube-master
    ...
    option tcplog
    balance source
    server 192.168.3.80 192.168.3.80:6443 check inter 2000 fall 2 rise 2 weight 1
    server 192.168.3.81 192.168.3.81:6443 check inter 2000 fall 2 rise 2 weight 1
    server 192.168.3.82 192.168.3.82:6443 check inter 2000 fall 2 rise 2 weight 1
EOF
```
...
+ master: 192.168.3.80
+ backup：192.168.3.81、192.168.3.82
...
``` bash
scp keepalived-master.conf root@192.168.3.80:/etc/keepalived/keepalived.conf
```

下发 backup 配置文件：

``` bash
scp keepalived-backup.conf root@192.168.3.81:/etc/keepalived/keepalived.conf
scp keepalived-backup.conf root@192.168.3.82:/etc/keepalived/keepalived.conf
```
